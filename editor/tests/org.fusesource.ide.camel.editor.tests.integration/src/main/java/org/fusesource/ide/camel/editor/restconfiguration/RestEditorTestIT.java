...
public class RestEditorTestIT extends AbstractCamelEditorIT {
	...
	public static String[] data() {
	    return new String[] { "camelContext" };
	}
	
	public RestEditorTestIT(String routeContainerType) {
	...
	public void openCamelFileMoveToRESTPage() throws Exception {
		...
		camelEditor.setActiveEditor(camelEditor.getRestEditor());
		assertThat(camelEditor.getActivePage() == CamelEditor.REST_CONF_INDEX);
		...
		assertThat(ssel.isEmpty()).isNotEqualTo(true);
		assertThat(ssel.getFirstElement() instanceof RestElement).isTrue();
		...
		assertThat(secondSelection.getId().contentEquals(initialSelection.getId()));
		...
		assertThat(opSsel1.isEmpty()).isNotEqualTo(true);
		...
		assertThat(initialRVE.getId().contentEquals(secondRVE.getId()));
		...
		assertThat(thirdSelection.getId().contentEquals(secondRestElement.getId()));
	}
}
